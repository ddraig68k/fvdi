...
//	if ((colour & 0xff) < 16)
//      	*foreground = (uint16_t) tos_colours[colour & 0x0f];
//	else if ((colour & 0xff) == 255)
//		*foreground = (uint16_t) 15;
//	else
//		*foreground = (uint16_t) colour & 0xff;
//
//        colour >>= 16;
//	if ((colour & 0xff) < 16)
//      	*background = (uint16_t) tos_colours[colour & 0x0f];
//	else if ((colour & 0xff) == 255)
//		*background = (uint16_t) 15;
//	else
//		*background = (uint16_t) colour & 0xff;
        *foreground = (uint16_t) tos_colours[colour & 0x0f];
        *background = (uint16_t) tos_colours[colour & 0x0f];
}
...
void CDECL c_set_colour (Virtual *vwk, long paletteIndex, long red, long green, long blue)
{
        ...
        colour.green = green;
        colour.blue = blue;
        v9990_WritePalette (&colour, (uint8_t) paletteIndex << 2, 1);
        v9990_WritePalette (&colour, (uint8_t) (32 + paletteIndex) << 2, 1); /* For B4-B7 modes mirror color data 0-31 to 32-63 */
}
