...
extern uint32_t g_gfxfpga_base;
...
static inline void drvga_bitmap_scroll_x(uint16_t x)
{
    DRVGA_REG_WRITE(REG_BITMAP_SCROLL_X, x);
}

static inline void drvga_bitmap_scroll_y(uint16_t y)
{
    DRVGA_REG_WRITE(REG_BITMAP_SCROLL_Y, y);
}
